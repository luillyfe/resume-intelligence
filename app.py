...
def run():
    ...
    uploaded_file = _upload_resume()

    if uploaded_file is not None:
        _process_resume(uploaded_file)
...
def _render_title():
    ...
    """
    st.title("📄 Resume Intelligence Platform with Roe AI")
...
def _process_resume(uploaded_file):
    """
    ...
    if hasattr(st.session_state, "insights"):
        _display_candidate_profile(st.session_state.insights)
        _setup_job_description_section(st.session_state.insights)

    if (
        hasattr(st.session_state, "show_scroll_message")
        and st.session_state.show_scroll_message
    ):
        st.sidebar.info("Please scroll down to see results")


def _extract_resume_insights(uploaded_file):
    """
...
def _setup_job_description_section(insights):
    """
    ...
    job_details = _extract_job_details(job_url)

    if job_details:
        _assess_job_match(job_details, insights)


def _extract_job_details(job_url):
    """
    ...
    if job_url:
        ...
        if hasattr(st.session_state, "job_details"):
            ...
            st.markdown(
                f"**Job Title:** {st.session_state.job_details.get('job_title', 'N/A')}"
            )
            st.markdown(
                f"**Location:** {st.session_state.job_details.get('location', 'N/A')}"
            )
            st.markdown(
                f"**Salary:** {st.session_state.job_details.get('salary', 'N/A')}"
            )
            st.markdown(
                f"**Equity:** {st.session_state.job_details.get('equity', 'N/A')}"
            )
            st.markdown(
                f"**Experience Level:** {st.session_state.job_details.get('experience_level', 'N/A')}"
            )

            st.subheader("Key Responsibilities")
            for responsabilities in st.session_state.job_details.get(
                "key_responsibilities", []
            ):
                st.markdown(f"- {responsabilities}")

            st.subheader("Requirements")
            for requirements in st.session_state.job_details.get("requirements", []):
...
def _assess_job_match(job_details, candidate_insights):
    """
        ...
        with st.spinner("Analyzing candidate-job match..."):
            try:
                ...
                if result:
                    ...
                    _render_job_match_assessment(match_assessment)
                else:
...
def _render_job_match_assessment(match_assessment):
    """
    ...
    with col2:
        _render_recommendation(match_assessment)
    ...
    _render_strengths(match_assessment)
    _render_skill_gaps(match_assessment)
...
def _render_recommendation(match_assessment):
    """
...
def _render_strengths(match_assessment):
    """
...
def _render_skill_gaps(match_assessment):
    """
...
def _display_candidate_profile(candidate_data: Dict[str, Any]):
    """
    ...
    with col2:
        ...
        _render_skills(candidate_data.get("skills", []))

    # Skills Proficiency Visualization
    _create_skills_chart(candidate_data.get("skills", []))
    ...
    if skills:
        ...
        for skill in skills:
            ...
            if proficiency is not None:
                proficiency_label = _get_proficiency_label(proficiency)
                skills_html += (
...
def _get_proficiency_label(proficiency):
    """
...
def _create_skills_chart(skills):
    """
    ...
    with tab2:
        # Enhanced Table View
        styled_df = df.style.background_gradient(cmap="Greens", subset=["proficiency"])
